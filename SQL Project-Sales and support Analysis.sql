...
-- In the table customer, column Phone_no holds inconsistent data, hence it can be undated.
...
/* The data type in ord_num is decimal, which is not required as no numerical calculations are to perfomed with ord_num, 
    hence modified to varchar data type. Same is applicable to grade column */
...
-- In this case to avoid duplicate values, columns can be converted to primary key
...
SELECT grade, 
       ...
       COUNT(*) * 100 / (SELECT COUNT(*) FROM customer) AS percentage
FROM customer
GROUP BY grade;
...
DELIMITER //
CREATE FUNCTION avg_amt(agent_name VARCHAR(100))
...
BEGIN
    ...
    --  agent ID based on the given agent_name
    SET agent_id = (SELECT agent_code FROM agents WHERE agent_name = agent_name LIMIT 1);
    
    -- Calculating the average outstanding amount for customers managed by the agent
    SET avg_outstanding_amt = (
        SELECT AVG(outstanding_amt) FROM customer WHERE agent_code = agent_id
    );
...
CREATE PROCEDURE cust_detail(IN para_grade INT)
BEGIN
    SELECT *
    FROM customer
    WHERE grade = para_grade;
END //
...
CREATE PROCEDURE sp_name(IN p_cust_code VARCHAR(10))
BEGIN
    DECLARE ord_nums VARCHAR(500) DEFAULT '';
    DECLARE done INT DEFAULT FALSE;
    DECLARE cur CURSOR FOR SELECT ord_num FROM orders WHERE cust_code = p_cust_code;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    
    OPEN cur;
    
    read_loop: LOOP
        FETCH cur INTO ord_nums;
        IF done THEN
            LEAVE read_loop;
        END IF;
        IF ord_nums != '' THEN
        ...
        END IF;
        SET ord_nums = CONCAT(ord_nums, cur.ord_num);
    END LOOP;
